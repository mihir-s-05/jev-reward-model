...
Before the full study, run the documented GPU preflight on the target hardware. It compares cached batched sampling with uncached training likelihoods and checks actor/critic backward gradients, without an optimizer update. Then run each judge's small counterexample audit to verify live authentication, schema, context budget, structured-output support, serving revision and usage. Finally perform a small grounded training run and checkpoint reload, followed by one run per evaluator arm, before scaling seeds/updates. These are runtime acceptance checks, not claims of work already performed.
