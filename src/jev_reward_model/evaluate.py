...
def main() -> None:
    ...
    ckpt = Path(args.checkpoint) if args.checkpoint else sorted(
        args.run_dir.glob("checkpoint-*"), key=lambda p: int(p.name.split("-")[-1])
    )[-1]
    processor = AutoProcessor.from_pretrained(args.model)
    base = AutoModelForMultimodalLM.from_pretrained(args.model, device_map="auto")
        ...
        rows.append(
            {
                ...
                "progress": env.state.progress,
                "violations": len(env.state.violations),
    ...
    summary = {
        ...
        "violation_rate": float(np.mean([r["violations"] > 0 for r in rows])),
