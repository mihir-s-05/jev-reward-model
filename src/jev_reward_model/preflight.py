"""User-run GPU compatibility check, NOT a training experiment or evaluator request."""
...
def main() -> None:
    ...
    del sampled
    if max(errors) > args.max_logprob_error:
    ...
    print(json.dumps({"status": "GPU preflight passed; no optimizer step or API call",
                      "joint_logprob_errors": errors, "revision": actor.resolved_revision,
                      "device": str(actor.device)}, indent=2))
